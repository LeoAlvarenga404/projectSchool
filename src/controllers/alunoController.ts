...
export const insertAluno = async (nome: string, cod_curso: number, email: string, data_nascimento: string, situacao_aluno: string): Promise<void> => {
  const pool = await conexao()
  await pool.request()
  .input('NOME', nome)
  ...
  .query(`
      INSERT INTO Aluno (nome, cod_curso, email, data_nascimento, situacao_aluno) 
      VALUES (@NOME, @COD_CURSO, @EMAIL, @DATA_NASCIMENTO, @SITUACAO_ALUNO);
    `)
}
...
export const updateAluno = async (cod_aluno: number, nome?: string, cod_curso?: number, email?: string, data_nascimento?: string, situacao_aluno?: string): Promise<void> => {
  const pool = await conexao();
  await pool.request()
  .input('COD_ALUNO', cod_aluno)
    .input('NOME', nome)
